...
module.exports = (databasePool) => {
    ...
    router.post('/register', async (req, res) => {
        ...
        if (!eventId || !registrationId) {
            return res.status(400).json({ error: 'Missing eventId or registrationId' }); // Validate input
        }

        try {
            // Insert the registration details into the 'registrations' table
            const query = 'INSERT INTO events_registrations (eventId, registrationId, createdAt) VALUES (?, ?, NOW())';
